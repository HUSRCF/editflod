...
def main() -> None:
    ...
    parser.add_argument("--translation-scale", type=float, default=1.0, help="Angstroms represented by one predicted translation unit")
    parser.add_argument("--rotation-scale", type=float, default=1.0, help="Radians represented by one predicted rotation unit")
    parser.add_argument("--delta-norm-weight", type=float, default=0.0, help="Optional penalty on predicted normalized update magnitude")
    parser.add_argument("--geometry-features", action="store_true", help="Include invariant chain and mutation-distance context features")
    ...
    parser.add_argument("--distill-weight", type=float, default=0.0, help="Weight of the optional teacher-delta loss")
    parser.add_argument("--max-normalized-delta", type=float, default=None, help="Optional tanh bound for each predicted normalized delta channel")
    ...
    selected = [
        ...
        if record.split == args.split and (args.allow_nonexperimental or record.label_source == "experimental")
    ]
    if not selected:
    ...
    if args.resume:
        ...
        if saved_geometry != args.geometry_features:
            raise SystemExit(
                f"resume checkpoint geometry_features={saved_geometry} does not match requested {args.geometry_features}"
            )
        ...
        if args.max_normalized_delta is not None:
            if saved_bound is None or not np.isclose(float(saved_bound), args.max_normalized_delta):
                raise SystemExit(
                    f"resume checkpoint max_normalized_delta={saved_bound} does not match requested {args.max_normalized_delta}"
                )
    ...
    if args.eval_split is not None:
        evaluation_records = [
            record
            for record in records
            if record.split == args.eval_split and (args.allow_nonexperimental or record.label_source == "experimental")
        ]
