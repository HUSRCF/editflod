...
def predict_student(
    ...
    *,
    ...
    parent_features = np.array(parent_cache.get(pair), copy=True) if parent_cache is not None else parent_local_features(pair, include_geometry=include_geometry)
    ...
    try:
        with torch.no_grad():
            ...
            if "edge_features" in parameters:
                ...
                edge_features, edge_mask = parent_spatial_graph(pair, spatial_neighbors=spatial_neighbors)
                kwargs["edge_features"] = torch.as_tensor(edge_features[None], dtype=torch.float32, device=device)
                kwargs["edge_mask"] = torch.as_tensor(edge_mask[None], dtype=torch.bool, device=device)
        ...
        with torch.no_grad():
            ...
            if "edge_features" in parameters:
                if edge_values is None or edge_masks is None:
                    raise ValueError("model requires include_spatial_graph=True")
                kwargs["edge_features"] = torch.as_tensor(edge_values, dtype=torch.float32, device=device)
                kwargs["edge_mask"] = torch.as_tensor(edge_masks, dtype=torch.bool, device=device)
            prediction = model(parent, edit, **kwargs)
    finally:
        if was_training:
            model.train()
